...
class RointeDeviceManager:
    ...
    async def update(self) -> dict[str, list[RointeDevice]]:
        ...
        for device_id in user_device_ids:
            ...
            futures = (
                ...
                self.hass.async_add_executor_job(
                    self.rointe_api.get_latest_energy_stats,
                    device_id,
        ...
        if firmware_map_response.success:
            # May be empty for Nexa users (no firmware endpoint)
            firmware_map: dict[RointeProduct, dict[str, str]] | None = (
                firmware_map_response.data or None
            )
        else:
            LOGGER.warning(
    ...
    async def _set_device_mode(self, device: RointeDevice, hvac_mode) -> bool:
        """Set the device hvac mode."""
        # Convert HVACMode enum to string if needed
        hvac_mode_str = hvac_mode.value if hasattr(hvac_mode, 'value') else str(hvac_mode)

        result = await self.hass.async_add_executor_job(
            self.rointe_api.set_device_mode, device, hvac_mode_str
        )
